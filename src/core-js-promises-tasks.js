...
function getAllResult(promises) {
  return Promise.allSettled(promises).then((arrPromise) => {
    return arrPromise.map((elem) => {
...
function queuePromises(promises) {
  return promises.reduce((acc, current) => {
    return acc.then((result) => current.then((value) => result + value));
  }, Promise.resolve(''));
}
